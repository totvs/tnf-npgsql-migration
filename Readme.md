...
Lembrando, os códigos das migrações, model da migração e ModelSnapshot são gerados especificamente para o provider que está sendo usando. Por isso não é possível usar migrações geradas com o provider do SqlServer num provider de PostgreSQL ou Oracle. Da mesma forma, não é possível usar as migrações geradas pelo provider DotConnect no provider da Npgsql, pois cada provider tem customizações e annotations especificas que ele usa ao gerar o script SQL da migração.
...
Contudo, esse processo não vai gerar um código que pode ser portado diretamente para o DbContext original da aplicação, pois sempre temos algumas diferenças entre os nomes no banco de dados e os nomes na model do DbContext, nome de relacionamentos, mapeamentos de alguns tipos e etc. Este processo seria bastante trabalhoso de se fazer manualmente e envolveria muita repetição, por isso desenvolvemos uma ferramenta e um pacote para auxilia-los.
...
* Ajuda no mapeamento dos nomes de campos e tabelas que estão no banco de dados, para os os nomes na model do DbContext;
* Ajuda no mapeamento de alguns tipos de dados no banco dados que acabam vindo diferentes da model original;
* Fornece algumas opções que são usadas durante o processo de migração.

Veremos mais detalhes do uso tanto da ferramenta quanto do pacote no tutorial a seguir.
...
A interface `IDesignTimeServices` implementada por essa classe é uma interface definida pelo EF Core. Ela permite registrar e substituir services que serão usadas nas operações de design time, tipo `Add-Migration`, `Update-Database`, `Scaffold-Context` e etc. Já o método de extensão `AddTnfEFCoreProviderMigration` é definido no pacote `Tnf.EntityFrameworkCore.Migration.Design` e faz o registro e substituição de services que precisamos para fazer essa migração. Dentro da lambda desse método também registramos os DbContext da aplicação que vamos migrar. O Registro é necessário porque durante o processo de Scaffold usamos a model do DbContext da aplicação para dar o nome correto às propriedades do DbContext gerados e não apenas no nome da coluna, como originalmente é feito.
...
Após a cópia vamos ter alguns erros de compilação porque o mapeamento das relações entre as entidades está diferente, pois no modelo original não temos a navegação bidirecional. Nesse caso basta apagar os mapeamentos que estão com erro, pois eles já são registrados no `OnModelCreating` do `BlogDbContext`.

Agora vamos olhar dentro da pasta `Migrations` do novo DbContext, o arquivo `ProviderMigration_PostgreSqlBlogDbContextModelSnapshot.cs`. Esse é o ModelSnapshot do `ProviderMigration_PostgreSqlBlogDbContext`. Ele também está em um namespace diferente. Então não podemos substituir o arquivo inteiro, ao invés disso, vamos apenas copiar os conteúdo do método `BuildModel` e substituir pelo conteúdo desse mesmo método no arquivo `PostgreSqlBlogDbContextModelSnapshot.cs`.
